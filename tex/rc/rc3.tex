...
\uncover<2>{
	\justifying
	\structb{Digression.} A Second Look into Connections of Distributions --- \texttt{s3.pdf}.
}
...
\uncover<4>{
	\justifying
	\structb{Proof (continued).} Applying the Chebyshev's inequality with $k = 2$ to 
	\begin{align*}
...
\justifying
\structb{Calculation of mean and variance.} Transform to Bernoulli trials $(X_1, \ldots, X_n)$.
\only<1>{
	\begin{itemize}
		\justifying
		\item The Bernoulli trials are identical with $p_k = \dfrac{r}{N}$, i.e.,
		\begin{align*}
		P[X_1 = 1] & = \frac{r}{N}, \\ 
		P[X_2 = 1] & = P[X_2 = 1|X_1 = 1]P[X_1 = 1] + \\
		& \qquad\qquad\qquad + P[X_2 = 1|X_1 = 0]P[X_1 = 0] \\
...
\only<2>{
	\begin{itemize}
		\justifying
		\item $\U{E}[X_k] = p_k = \dfrac{r}{N}, \U{Var}[X_k] = p_k(1-p_k)$.
		\item For variance,
		\begin{align*}
		\U{Var}[X] = \sum_{k=1}^n \U{Var}[X_k] + 2\sum_{i<j}\U{Cov}[X_i, X_j],
		\end{align*}
		where
		\begin{align*}
		\U{Cov}[X_i, X_j] & = \red{\U{E}[X_iX_j]} - \U{E}[X_i]\U{E}[X_j], \\
		\red{\U{E}[X_i, X_j]} & = p_{ij} = \frac{r}{N}\cdot \frac{r-1}{N-1}, \qquad i\neq j.
		\end{align*}
	\end{itemize}
}
...
\only<2>{
	...
	\end{align*}
	How to calculate $\U{Cov}[X_i, X_j]$?
}
\only<3>{
	\justifying
	\structb{Solution (approach 1).} Knowing that
	\begin{align*}
	\U{E}[X_iX_j] = \frac{2}{T(T-1)}\sum_{i<j} a_ia_j,
	\end{align*}
	and
	\begin{align*}
	\U{Var}[X_i] & = \frac{1}{T}\sum_{i=1}^T (a_i-\mu)^2  = \frac{1}{T}\sum_{i=1}^T (a_i^2 - 2\mu a_i + \mu^2) \\
	& = \frac{1}{T}\left[\left(\sum_{i=1}^T a_i^2\right) - 2T\mu^2 + T\mu^2\right] \\
	& = \frac{1}{T}\sum_{i=1}^T a_i^2 - \mu^2.
	\end{align*}
}
\only<4>{
	\justifying
	\structb{Solution (approach 1).} Then
	\footnotesize
	\begin{align*}
	\U{Cov}[X_i, X_j] & = \frac{2}{T(T-1)}\sum_{i<j} a_ia_j - \frac{1}{T^2}\left(\sum_{i=1}^T a_i \right)^2 \\
	& = \frac{1}{T^2(T-1)}\left[2T\sum_{i<j} a_ia_j - (T-1)\left(\sum_{i=1}^T a_i^2 + 2\sum_{i<j}a_ia_j \right) \right] \\
	& = \frac{1}{T^2(T-1)}\left[\left(\sum_{i=1}^T a_i \right)^2 - \sum_{i=1}^Ta_i^2 - (T-1)\sum_{i=1}^T a_i^2 \right] \\
	& = \frac{1}{T^2(T-1)} \left[T^2\mu^2 - T^2\sigma^2-T^2\mu^2 \right] = -\frac{\sigma^2}{T-1}.
	\end{align*}
}
\uncover<5>{
	\justifying
	\structb{Solution (approach 2).} Because $\U{Cov}[X_i, X_j]$ does not depend on $i, j$ as long as $i\neq j$, we have
	\begin{align*}
