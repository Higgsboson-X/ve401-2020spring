...
\only<4>{
	\begin{itemize}
		...
		\Rightarrow & n \approx \frac{(z_{\alpha} + z_{\beta})^2}{\delta^2}.
		\end{align*}
	\end{itemize}
}
\uncover<5>{
...
\justifying
\structb{Chi-squared test.} Let $X_1, \ldots, X_n$ be a random sample of size $n$ from a normal distribution with \highlightr{unknown} variance $\sigma^2$. Let $\sigma^2_0$ be a null value of the variance. Then the test statistic is given by
\begin{align*}
