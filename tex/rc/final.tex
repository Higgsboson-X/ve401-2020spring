...
\end{itemize}
\highlightr{Note:} Refer to an estimator of the parameter in the null hypothesis ($\widehat{\mu} = \overline{X}$ in this case). \\
\uncover<2>{
	\justifying
...
\end{align*}
\normalsize
However, with \underline{equal} sizes, \highlightr{$f_2 = 1/f_1$}, and thus
\footnotesize
\begin{align*}
...
\begin{itemize}
	\justifying
	\item \structb{Setting up $H_0$.} In most of the cases, when we want to find evidence \highlightr{for} an event, $H_0$ is usually set up to be the opposite of this event. \\
